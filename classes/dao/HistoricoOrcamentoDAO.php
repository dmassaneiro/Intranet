...
class HistoricoOrcamentoDAO {

    public function Inserir(HistoricoOrcamento $conserto) {
        try {
            $sql = "INSERT INTO historico_orcamento (    
                  data,
                  orcamento_id,
                  usuario,
                  operacao)
            ...
            $p_sql->bindValue(":orcamento_id", $conserto->getConserto_id());
    ...
    public function BuscarTodos($id) {
        try {
            $sql = "SELECT * FROM historico_orcamento WHERE orcamento_id = $id";
    ...
    private function Lista($row) {
        $post = new HistoricoOrcamento();

        $post->setId($row['id']);
        $post->setData($row['data']);
        $post->setUsuario($row['usuario']);
        $post->setConserto_id($row['orcamento_id']);
